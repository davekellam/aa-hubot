...
module.exports = (robot) ->
  ...
  robot.hear /(@\S+)[ ]*\+\+(\s|$)/, (msg) ->
    ...
    if ( subject != user )
      ...
      msg.send "#{subject} #{karma.incrementResponse()} (Validation: '&radic;-'#{Math.floor(Math.random() * 100)})"
    else
  ...
  robot.hear /(@\S+)[ ]*--(\s|$)/, (msg) ->
    ...
    if ( subject != user )
      ...
      msg.send "#{karma.decrementResponse()} #{subject} (Validation: '&radic;-'#{Math.floor(Math.random() * 100)})"
    else
      msg.send "#{subject} Naughty naughty, no self-karma."
  ...
  robot.hear /(@\S+)[ ]*(\+\-|\-\+)(\s|$)/, (msg) ->
    user = "@" + msg.message.user.mention_name
    subject = msg.match[1].toLowerCase()
    msg.send "#{subject} #{karma.neutralResponse()} (Validation: '&radic;-'#{Math.floor(Math.random() * 100)})"
