# Description:
#   Synergistic management solutions for tomorrow, today!
#
# Dependencies:
#   None
#
...
#   !buzzwords
#   !jargon
#   hubot generate meeting topic
#
# Author:
#   stevegrunwell
#
# Based on www.atrixnet.com/bs-generator.html
...
nouns = [
	...
	'testing procedures', 'total linkage', 'users', 'value', 'vortals', 'web-readiness', 'web services', 'fungibility', 'clouds',
	'nosql', 'storage'
]
...
turn_a_phrase = (msg) ->
	words = [
		msg.random adverbs
		msg.random verbs
		msg.random adjectives
		msg.random nouns
	]
	return words.join(' ')

module.exports = (robot) ->
  robot.hear /^!(buzzwords?|jargon)/i, (msg) ->
    msg.send turn_a_phrase(msg)

  robot.respond /(generate )?(me )?meeting topic/i, (msg) ->
    msg.send "In this meeting, we shall discuss how to " + turn_a_phrase(msg)
